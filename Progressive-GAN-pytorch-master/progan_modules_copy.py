...
class Generator(nn.Module):
    def __init__(self, input_code_dim=128, in_channel=128, pixel_norm=True, tanh=True, n_cond_lyrs=64):
        super().__init__()
        ...
        self.input_layer = nn.Sequential(
            nn.Linear(input_code_dim, 4 * 4 * in_channel)
        )
        ...
        self.progression_256 = ConvBlock(in_channel + n_cond_lyrs, in_channel, 3, 1, pixel_norm=pixel_norm)
        ...
        self.to_rgb_256 = EqualConv2d(in_channel, 1, 1)
    ...
    def forward(self, input, raw_cond_128, step=0, alpha=-1):
        ...
        out_4 = out_4.reshape(-1, self.in_channel, 4, 4)
...
class Discriminator(nn.Module):
    def __init__(self, feat_dim=128):
        ...
        self.progression = nn.ModuleList([ConvBlock(feat_dim // 4, feat_dim // 4, 3, 1),
                                          ...
                                          ConvBlock(feat_dim, feat_dim, 3, 1),
                                          ConvBlock(feat_dim, feat_dim, 3, 1),
                                          ConvBlock(feat_dim + 1, feat_dim, 3, 1, 2, 0)])
...
if __name__ == "__main__":
    ...
    batch_size = 64
    cond_array = torch.zeros(batch_size, 1, 128, 128, device='cuda:0') * 2 - 1
    gen_z = torch.randn(batch_size, 128).to('cuda:0')
    ...
    fake_image = generator(gen_z, cond_array, step=6, alpha=0.02)
    d_result = discriminator(fake_image, step=6, alpha=0.02)
    breakpoint()
