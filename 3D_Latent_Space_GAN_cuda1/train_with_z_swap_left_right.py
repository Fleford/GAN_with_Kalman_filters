...
def train(generator, discriminator, init_step, loader, total_iter=600000, max_step=7):      # max_step=7)
    ...
    for i in pbar:
        ...
        # second half of the batch is a left-swapped version of the first half
        ...
        if (i + 1) % n_critic == 0:
            ...
            cond_mask[:, :, :, 0:cond_mask.shape[3] // 8] = 1
            cond_mask[:, :, :, -cond_mask.shape[3] // 8:] = 1
            ...
            loss = -predict.mean() + 0.04 * context_loss_value
            # loss = -predict.mean()
        ...
        if (i + 1) % 1000 == 0 or i == 0:
            with torch.no_grad():
                ...
                sample_z_left_swap = torch.flip(sample_z_first_half[:, :, :, 0:sample_z_first_half.shape[3] // 2], dims=[0])
                sample_z_right = sample_z_first_half[:, :, :, sample_z_first_half.shape[3] // 2:sample_z_first_half.shape[3]]
                sample_z_second_half = torch.cat((sample_z_left_swap, sample_z_right), dim=3)
