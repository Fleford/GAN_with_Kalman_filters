...
class Generator(nn.Module):
    def __init__(self, input_z_channels=9, in_channel=128, pixel_norm=True, tanh=True, n_cond_lyrs=4):
        super().__init__()
        # self.input_dim = input_z_channels
        ...
        self.progression_2 = ConvBlock(input_z_channels, in_channel, 3, 1, pixel_norm=pixel_norm)
        ...
        self.progression_64 = ConvBlock(in_channel, in_channel, 3, 1, pixel_norm=pixel_norm)
        self.progression_128 = ConvBlock(in_channel, in_channel, 3, 1, pixel_norm=pixel_norm)
        ...
        self.to_rgb_2 = EqualConv2d(in_channel, 1, 1)
        self.to_rgb_4 = EqualConv2d(in_channel, 1, 1)
        ...
        self.to_rgb_64 = EqualConv2d(in_channel, 1, 1)
        self.to_rgb_128 = EqualConv2d(in_channel, 1, 1)
    ...
    def forward(self, input, step=0, alpha=-1):
        ...
        # # testing dimensions
        # out_2 = self.progression_2(input)
        # step_1_out = self.to_rgb_2(out_2)
        #
...
if __name__ == "__main__":
    ...
    # cond_array = torch.zeros(4, 1, 128, 128, device='cuda:1') * 2 - 1
    # gen_z = torch.randn(4, 128).to('cuda:1')
    gen_z = torch.randn(4, 9, 2, 2).to('cuda:1')    # 4 z vectors of 2x2 with 9 channels
    generator = Generator(in_channel=128, input_z_channels=9, pixel_norm=False, tanh=False).to('cuda:1')
    discriminator = Discriminator(feat_dim=128).to('cuda:1')
    fake_image = generator(gen_z, step=7, alpha=0.02)
